 ...
 where
  ...
  prf p ucp t uct = ∃-intro l pr
   where
    ...
    eq α = ∃-elim ucp α (cons (take m α) 0̄) (Lemma[cons-take-≣_≣] m α 0̄)
    k' : ℕ
    k' = ∃-witness (max-fin f)
    k'-max : ∀(α : ₂ℕ) → p α ≤ k'
    k'-max α = subst {ℕ} {λ i → i ≤ k'} (sym (eq α)) (∃-elim (max-fin f) (take m α))
    k : ℕ
    k = succ k'
    k-max : ∀(α : ₂ℕ) → p α < k
    ...
    pr α β awl = subst {ℕ} {λ i → r(t α)(p α) ≡ r(t β) i} eqp subgoal
     where
      awm : α ≣ m ≣ β
      awm i i<m = awl i (Lemma[a<b≤c→a<c] i<m m≤l)
      eqp : p α ≡ p β
      eqp = ∃-elim ucp α β awm
      awn : α ≣ n ≣ β
      awn i i<n = awl i (Lemma[a<b≤c→a<c] i<n n≤l)
      awk : r (t α) ≣ k ≣ r (t β)
      awk = ∃-elim (ucrt k) α β awn
      subgoal : r(t α)(p α) ≡ r(t β)(p α)
      subgoal = awk (p α) (k-max α)
