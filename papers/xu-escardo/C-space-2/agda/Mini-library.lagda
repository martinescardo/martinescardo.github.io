...
-- Bracket types, to formalize proof irrelevance
...
Lemma[∘-constant] f g cf = hide (λ x x' → cong g (reveal cf x x'))
...
Lemma[id-UC] m = m , hide (λ α β eq → eq)
...
Lemma[∘-UC] s ucs t uct m = ∃-intro mt est
 where
  ...
  est = hide (λ α β aw → reveal es (t α) (t β) (reveal et α β aw))
