...
holds→equal-true : {i : 𝕃} {p : Prp{i}} → p holds → p ≡ true
holds→equal-true {i} {p} h = Prop-≡ (prop-ua (holdsIsProp p) (holdsIsProp true) (λ _ → zero) (λ _ → h))

equal-true→holds : {i : 𝕃} {p : Prp{i}} → p ≡ true → p holds
...
_∧_ _∨_ _⇒_ : {i : 𝕃} → Prp{i} → Prp{i} → Prp{i}
...
∀̇ ∃̇ : {i j : 𝕃} → {X : U i} → (X → Prp{j}) → Prp{i ⊔ j}
...
∧-intro : {i : 𝕃} {p q : Prp{i}} → p holds → q holds → p ∧ q holds
∧-intro = _,_

∨-intro-L : {i : 𝕃} {p q : Prp{i}} → p holds → p ∨ q holds
∨-intro-L h = ∣ inl h ∣

∨-intro-R : {i : 𝕃} {p q : Prp{i}} → q holds → p ∨ q holds
∨-intro-R k = ∣ inr k ∣

⇒-intro : {i : 𝕃} {p q : Prp{i}} → (p holds → q holds) → p ⇒ q holds
⇒-intro = λ f → f

∃̇-intro : {i j : 𝕃} {X : U i} {p : X → Prp{j}} (x : X) → p x holds → ∃̇ p holds
∃̇-intro x h = ∣ x , h ∣
...
∧-elim-L :  {i : 𝕃} {p q : Prp{i}} → p ∧ q holds → p holds
∧-elim-L = pr₁

∧-elim-R :  {i : 𝕃} {p q : Prp{i}} → p ∧ q holds → q holds
∧-elim-R = pr₂

false-elim : {i j : 𝕃} {p : Prp{i}} → false {j} holds → p holds
false-elim = λ()

∨-elim :  {i : 𝕃} {p q r : Prp{i}} → (p holds → r holds) → (q holds → r holds)
       → p ∨ q holds → r holds
∨-elim {i} {p} {q} {r} f g = ∥∥-rec (holdsIsProp r) (+-rec f g)

⇒-elim : {i : 𝕃} {p q : Prp{i}} → (p holds → q holds) → p holds → q holds
⇒-elim = λ f → f

∃̇-elim : {i j : 𝕃} {X : U i} {p : X → Prp{j}} {r : Prp{i ⊔ j}}
        → ((x : X) → p x holds → r holds)
        → ∃̇ p holds → r holds
∃̇-elim {i} {j} {X} {p} {r} f = ∥∥-rec (holdsIsProp r) (Σ-rec f)

∀̇-elim : {i j : 𝕃} {X : U i} {p : X → Prp{j}} → ∀̇ p holds → (x : X) → p x holds
∀̇-elim = λ f → f

-- Characterizations in terms of _⇒_ and ∀̇:

_⟷_ : {i j : 𝕃} → Prp{i} → Prp{j} → U (i ⊔ j)
...
∧-charac : {i : 𝕃} (p q : Prp{i}) → (p ∧ q) ⟷ ∀̇ \r → (p ⇒ q ⇒ r) ⇒ r
∧-charac {i} p q = (a , b)
 where
...
∨-charac : {i : 𝕃} (p q : Prp{i}) → (p ∨ q) ⟷ ∀̇ \r → (p ⇒ r) ⇒ (q ⇒ r) ⇒ r
∨-charac {i} p q = (a , b)
 where
