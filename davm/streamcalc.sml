...
fun flip'(0::alist,0,n) = [0]@n_ones(n)@[0]@alist          (* 0 1^n *0* alist *)
...
and flip(0::alist,0) = (0::alist)
  | flip(0::alist,d) = flip'(alist,d-1,0)
  | flip(1::alist,_) = raise BUG;

(******************************************************************)
(* Normalising 2 streams, makes exponent equal *)
...
(* Addition for simplified notation *)
...
(* Zip function that combines two streams by adding each digit together *)
fun zip (cons(a,s1), cons(b,s2)) = 
    cons((a+b):int, fn() => zip(force s1, force s2));
(* Zip function multiplying each digit by 2 *)
...
(* function rewriting a stream containing 0's and 2's, into a stream
containing only 0's and 1's *)
  ...
  | dbl _ = raise BUG;
in
(* Function implementing multiplication by 2 in full notation *)
    fun double(cons(z,s))= 
...
(* New implementation of addition (the first was designed by Di Gianantonio) *)
...
(* New algorithm for addition in full notation *)
...
(* This function takes in an integer and calculates in GR notation this value *)
...
fun flip_first_simp (cons(0, s), n)=
	...
	val (sign::list) = flip([0]@(working_list)@[0,0], 1) in 
	if (sign=1) then (* sign=1 => value of list is positive *)
		...
		val (overflow::one::zero::x::y::rest) = flip([0]@negate_list@[0,0], 2)
	    in
