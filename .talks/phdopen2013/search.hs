...
-- Some more Haskell code for phdopen'2012, Warsaw University,
...
findCantorI p = if p(Zero # findCantorI(\a -> p(Zero # a)))
                then Zero # findCantorI(\a -> p(Zero # a))
                else  One # findCantorI(\a -> p( One # a))                    
...
findCantoR :: (Sequence Bool -> Bool) -> Sequence Bool
findCantoR p = x # a 
  where x = p(True # findCantoR(\a -> p(True # a)))
        a = findCantoR(\a -> p(x # a)) 
