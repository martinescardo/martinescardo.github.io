...
This version is slightly adapted for teaching at Proof and Computation 2018
https://www.mathematik.uni-muenchen.de/~schwicht/pc18.php

\begin{code}

module laconic-dialogue-pc2018 where
...
continuity-extensional : (f g : Baire → ℕ) → ((α : Baire) → f α ≡ g α)
                      → continuous f → continuous g
continuity-extensional f g t c α = (π₀(c α) , (λ α' r → trans (sym (t α)) (trans (π₁(c α) α' r) (t α'))))

\end{code}

The same proof written in a more readable way:

\begin{code}

continuity-extensional' : (f g : Baire → ℕ) → ((α : Baire) → f α ≡ g α)
                       → continuous f → continuous g
continuity-extensional' f g t c α = (s , n)
 where
  s : List ℕ
  s = π₀(c α)
  m : (α' : Baire) → α ≡[ s ] α' → f α ≡ f α'
  m = π₁(c α)
  n : (α' : Baire) → α ≡[ s ] α' → g α ≡ g α'
  n α' r = g α  ≡⟨ sym (t α) ⟩
           f α  ≡⟨ m α' r ⟩
           f α' ≡⟨ t α' ⟩
           g α' ∎
...
The same proof in a more readable form:

\begin{code}

eloquent-is-continuous' : (f : Baire → ℕ) → eloquent f → continuous f
eloquent-is-continuous' f (d , e) = γ
 where
  g : Baire → ℕ
  g = dialogue d
  NB : (α : ℕ → ℕ) → g α ≡ f α
  NB = e
  c : continuous g
  c = dialogue-continuity d
  γ : continuous f
  γ = continuity-extensional g f e c
...
rec' {σ} f x = kleisli-extension' blah
 where
  blah : ℕ → B-Set⟦ σ ⟧
  blah = rec f x
